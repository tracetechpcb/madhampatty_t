...
mysql -h $DB_CONTAINER_NAME -u $DB_ROOT_USER --password=$DB_ROOT_PASSWORD $DB_NAME -e "$DELETE_LICENSE_CMD"
...
mysql -h $DB_CONTAINER_NAME -u $DB_ROOT_USER --password=$DB_ROOT_PASSWORD $DB_NAME -e "$INSERT_LICENSE_CMD"
...
VALUES ('$DEFAULT_APPLICATION_USERNAME', '$DEFAULT_APPLICATION_PASSWORD', NULL, 1, 'Admin', 'Tracetech', 'admin@tracetech.com', 0, 1, NOW())
ON DUPLICATE KEY UPDATE username=VALUES(username);"
echo $INSERT_DEFAULT_USER_CMD
mysql -h $DB_CONTAINER_NAME -u $DB_ROOT_USER --password=$DB_ROOT_PASSWORD $DB_NAME -e "$INSERT_DEFAULT_USER_CMD"